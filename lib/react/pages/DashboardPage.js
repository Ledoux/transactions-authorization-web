...
var DashboardPage = function (_Component) {
  ...
  function DashboardPage() {
    ...
    _this.state = { visibleModes: [] };
    _this.handleBackHome = _this._handleBackHome.bind(_this);
    _this.handleSetVisibleModes = _this._handleSetVisibleModes.bind(_this);
    return _this;
  }
  ...
  _createClass(DashboardPage, [{
    ...
    value: function _handleBackHome(prevProps) {
      ...
      var visibleModes = this.state.visibleModes;
      ...
      if (window.location.pathname === pathName) {
        ...
        if (ModalHomeComponent) {
          ...
          var nextSearch = (0, _transactionsInterfaceState.getLocationSearchString)(Object.assign({ selectedModeName: selectedMode.name }, search));
          history.push({
            search: nextSearch
          });
    ...
    key: '_handleSetVisibleModes',
    value: function _handleSetVisibleModes(props) {
      var availableModes = props.availableModes,
          modeNames = props.modeNames,
          excludedModeNames = props.excludedModeNames;

      var visibleModes = modeNames && availableModes && availableModes.filter(function (_ref2) {
        var name = _ref2.name;
        return modeNames.includes(name);
      }) || availableModes && availableModes.filter(function (_ref3) {
        var name = _ref3.name;
        return !excludedModeNames.includes(name);
      });
      if (visibleModes) {
        this.setState({ visibleModes: visibleModes });
      }
    }
  }, {
          ...
          visibleModes && visibleModes.map(function (_ref4, index) {
            ...
            return DashboardComponent && _react2.default.createElement(
              'div',
              {
                ...
                key: index },
              _react2.default.createElement(DashboardComponent, transactionsProps)
            );
          })
        )
      );
...
function mapStateToProps(_ref5) {
  var _ref5$authorization = _ref5.authorization,
      availableModes = _ref5$authorization.availableModes,
